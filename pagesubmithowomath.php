...
if(isset($_POST["submithomework"])) {
    ...
    if ($errorMessage == "") {
        ...
        try {
            ...
            $stmt = $dbh->prepare("SELECT filename FROM images WHERE subject = :subject AND imagetype = :imagetype AND date = :date AND filename = :filename AND studentid = :studentid LIMIT 1");
